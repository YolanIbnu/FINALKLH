...
export function AddStaffModal({ report, profiles, onClose }: AddStaffModalProps) {
  const { state } = useApp()
  ...
  const alreadyAssignedStaffIds = report.task_assignments?.map((a: any) => a.staff_id) || []
  const availableStaff = profiles.filter((p: any) => p.role === 'Staff' && p.id && !alreadyAssignedStaffIds.includes(p.id))
  ...
  const handleTodoChange = (todoTask: string, checked: boolean) => {
          ...
          {alreadyAssignedStaffIds.length > 0 && (
            <div className="bg-gray-50 p-3 rounded-lg border">
              ...
              <ul className="list-disc list-inside text-sm text-gray-600">
                {alreadyAssignedStaffIds.map((id: string) => (
